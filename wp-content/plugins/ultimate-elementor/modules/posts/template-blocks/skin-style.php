...
abstract class Skin_Style {
	...
	public function render_featured_title() {
		...
		<?php if ( $this->get_instance_value( 'link_title' ) ) { ?>
			...
			<a href="<?php echo apply_filters( 'uael_single_post_link', get_the_permalink(), get_the_ID(), $settings ); ?>" target="<?php echo $target; ?>">
				<?php the_title(); ?>
			</a>
	...
	public function render_featured_meta_data() {
		...
		<<?php echo $this->get_instance_value( 'meta_tag' ); ?> class="uael-post__meta-data">
	...
	public function render_featured_excerpt() {
		...
		add_filter( 'excerpt_length', [ $this, 'uael_featured_excerpt_length_filter' ], 20 );
		add_filter( 'excerpt_more', [ $this, 'uael_excerpt_more_filter' ], 20 );
		...
		remove_filter( 'excerpt_length', [ $this, 'uael_featured_excerpt_length_filter' ], 20 );
		remove_filter( 'excerpt_more', [ $this, 'uael_excerpt_more_filter' ], 20 );
	...
	public function render_title() {
		...
		do_action( 'uael_single_post_before_title', get_the_ID(), $settings );
		?>
		<<?php echo $this->get_instance_value( 'title_tag' ); ?> class="uael-post__title">
	...
	public function render_date() {
		...
		<span class="uael-post__date">
			...
			<i class="<?php echo $icon; ?>"></i>
			<?php } ?>
			<?php echo apply_filters( 'uael_post_the_date_format', get_the_date(), get_the_ID(), get_option( 'date_format' ), '', '' ); ?>
		</span>
		<?php
	...
	public function get_meta_html_by_prefix( $terms, $prefix ) {
		...
		printf( '<span class="uael-post__terms-meta uael-post__terms-meta-%s">%s</span>', $prefix, $result );
	...
	public function render_term_html() {
		...
		if ( '' != $this->get_instance_value( 'terms_to_show' ) ) {
		...
		printf( '<span class="uael-post__terms">%s</span>', $result );
	...
	public function render_excerpt() {
		...
		add_filter( 'excerpt_length', [ $this, 'uael_excerpt_length_filter' ], 20 );
		add_filter( 'excerpt_more', [ $this, 'uael_excerpt_more_filter' ], 20 );
		...
		remove_filter( 'excerpt_length', [ $this, 'uael_excerpt_length_filter' ], 20 );
		remove_filter( 'excerpt_more', [ $this, 'uael_excerpt_more_filter' ], 20 );
	...
	public function render_pagination() {
		...
		if ( ! in_array( $this->get_instance_value( 'post_structure' ), [ 'normal', 'featured', 'masonry' ] ) ) {
			return;
		}
		...
		if ( $total_pages > 1 ) {
			...
			?>
			<nav class="uael-grid-pagination" <?php echo $class; ?> role="navigation" aria-label="<?php _e( 'Pagination', 'uael' ); ?>" data-total="<?php echo $total_pages; ?>">
			...
			<div class="uael-post__load-more-wrap">
				<a class="uael-post__load-more elementor-button" href="javascript:void(0);">
					<span><?php echo $this->get_instance_value( 'load_more_text' ); ?></span>
				</a>
			</div>
				<?php
			}
	...
	public function render_filters() {
		...
		?>
		<div class="uael-post__header-filters-wrap<?php echo $tab_responsive; ?>">
			<ul class="uael-post__header-filters">
				<li class="uael-post__header-filter uael-filter__current" data-filter="*"><?php echo ( 'All' == $all || '' == $all ) ? __( 'All', 'uael' ) : $all; ?></li>
				<?php foreach ( $filters as $key => $value ) { ?>
				<li class="uael-post__header-filter" data-filter="<?php echo '.' . $value->slug; ?>"><?php echo $value->name; ?></li>
				<?php } ?>
			</ul>

			<?php if ( 'yes' == $this->get_instance_value( 'tabs_dropdown' ) ) { ?>
				<div class="uael-filters-dropdown">
					<div class="uael-filters-dropdown-button"><?php echo ( 'All' == $all || '' == $all ) ? __( 'All', 'uael' ) : $all; ?><i class="fa fa-angle-down"></i></div>

					<ul class="uael-filters-dropdown-list uael-post__header-filters">
						<li class="uael-filters-dropdown-item uael-post__header-filter uael-filter__current" data-filter="*"><?php echo ( 'All' == $all || '' == $all ) ? __( 'All', 'uael' ) : $all; ?></li>
						<?php foreach ( $filters as $key => $value ) { ?>
						<li class="uael-filters-dropdown-item uael-post__header-filter" data-filter="<?php echo '.' . $value->slug; ?>"><?php echo $value->name; ?></li>
						<?php } ?>
					</ul>
				</div>
			<?php } ?>
