...
class CafStyler extends Common_Widget {
	...
	public function get_script_depends() {
		return [ 'uael-frontend-script', 'uael-caf-styler' ];
	}
	...
	public function uael_select_caldera_forms() {
		global $wpdb, $forms;
		$uael_cf_table_name = $wpdb->prefix . 'cf_forms';
		$forms              = $wpdb->get_results( "SELECT * FROM $uael_cf_table_name" ); // WPCS: unprepared SQL OK.
		$options['-1']      = 'Select';
	...
	protected function register_button_content_controls() {
		...
		$this->add_responsive_control(
			...
			[
				...
				'size_units' => [ 'px', 'em', '%' ],
				'selectors'  => [
					'{{WRAPPER}} .uael-caf-form input[type="submit"], {{WRAPPER}} .uael-caf-form .cf-uploader-trigger, {{WRAPPER}} .uael-caf-form a.btn-default' => 'padding: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};',
				],
				'separator'  => 'after',
			]
		);
		...
		if ( parent::is_internal_links() ) {
			...
			$this->add_control(
				...
				[
					...
					'raw'             => sprintf( __( '%1$s Getting started article » %2$s', 'uael' ), '<a href="https://uaelementor.com/docs/caldera-form-styler-widget/?utm_source=uael-pro-dashboard&utm_medium=uael-editor-screen&utm_campaign=uael-pro-plugin" target="_blank" rel="noopener">', '</a>' ),
			...
			$this->add_control(
				...
				[
					'type'            => Controls_Manager::RAW_HTML,
					/* translators: %1$s Doc Link */
					'raw'             => sprintf( __( '%1$s Unable to See Caldera Form Styler Widget? » %2$s', 'uael' ), '<a href="https://uaelementor.com/docs/unable-to-see-caldera-form-styler-widget/?utm_source=uael-pro-dashboard&utm_medium=uael-editor-screen&utm_campaign=uael-pro-plugin" target="_blank" rel="noopener">', '</a>' ),
					'content_classes' => 'uael-editor-doc',
				]
			);
